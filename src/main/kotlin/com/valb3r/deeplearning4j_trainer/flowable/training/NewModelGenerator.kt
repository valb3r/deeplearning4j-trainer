...
import com.valb3r.deeplearning4j_trainer.samediff.SdDenseLayerFactory
import com.valb3r.deeplearning4j_trainer.samediff.SirenLayerFactory
import com.valb3r.deeplearning4j_trainer.samediff.SlowFourierTransformLayerFactory
...
class NewModelGenerator: WrappedJavaDelegate() {
    ...
    override fun doExecute(execution: DelegateExecution) {
        ...
        for (layer in modelSpec.layers) {
            prevLayer = when (layer) {
                ...
                is FftAmplitudeViewLayerDef -> makeFftAmplitudeViewLayer(sd, layer)
    ...
    private fun makeFftAmplitudeViewLayer(sd: SameDiff, layerDef: FftAmplitudeViewLayerDef): SDVariable {
        val builder = SlowFourierTransformLayerFactory.builder {
            this.sd = sd
            this.nIn = layerDef.`in`
            this.dataType = DataType.FLOAT
        }

        val input: SDVariable = extractVarExpr(sd, layerDef.inputExpression)
        val built = builder.build()
        val name = sd.uniqName(arrayOf(layerDef.outputVarName, layerDef.name).filterNotNull().first())
        return built.createAmplitudeView(name, input).rename(name)
    }
    ...
    private fun makeLoss(sd: SameDiff, loss: Loss) {
        when (loss.type) {
            ...
            "mae" -> makeMaeLoss(sd, loss.entries)
            else -> throw IllegalArgumentException("Unknown loss type ${loss.type}")
        }
    }

    private fun makeMaeLoss(sd: SameDiff, entries: List<LossItem>) {
        if (entries.isEmpty()) {
        ...
        for (entry in entries) {
            lossValue = lossValue.add(
                sd.mean(
                    sd.math.abs(sd.math.sub(extractVarExpr(sd, entry.varName), sd.getVariable(entry.labelName)))
                        .rename(sd.uniqName("abs-diff-${entry.varName}-${entry.labelName}"))
                ).mul(entry.weight ?: 1.0).rename(sd.uniqName("mae-${entry.varName}-${entry.labelName}"))
            )
        }
